...
use casper_types::{EntryPoint, ContractHash, Key, EntryPointAccess, CLType::{U64, URef, self}, EntryPointType, EntryPoints, contracts::NamedKeys, RuntimeArgs, runtime_args, account::AccountHash, Parameter, U256};
...
pub fn only_owner() {
    ...
    if admin != caller { runtime::revert(Error::AdminError) }
}
...
pub fn reward_per_token() -> u64 {
    let pool: u64 = utils::read_from(POOL);
    ...
    reward_per_token_stored + (reward_rate * (last_time_reward_applicable() - update_at) * (decimal as u64)) / pool
}
...
pub extern "C" fn stake() {
    ...
    let mut pool: u64 = utils::read_from(POOL);
    ...
    pool = pool + amount.as_u64();
    runtime::put_key(POOL, storage::new_uref(pool).into());
}
...
pub extern "C" fn call() {
    ...
    named_keys.insert(POOL.to_string(), storage::new_uref(pool).into());
